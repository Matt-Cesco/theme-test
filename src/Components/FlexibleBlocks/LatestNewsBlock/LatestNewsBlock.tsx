"use client";

import React, { useEffect, useRef, useState } from "react";
import { motion } from "framer-motion";
import gsap from "gsap";
import ILatestNewsBlock from "./ILatestNewsBlock"; // Updated import
import IFlexibleBlock from "../IFlexibleBlock";
import Link from "next/link";
import { getLatestNews } from "@/Graphql/wordpressCMS/queries/getLatestNews";
import FormatDate from "@/Common/FormatDate";
import { MediaItem } from "@/Graphql/generated";
import DynamicImage from "@/Common/DynamicImage/DynamicImage";
import HoverPortal from "@/Components/Layout/HoverPortal/HoverPortal";
import AnimatedLink from "@/Components/Layout/AnimatedLink";

type InsightsItem = {
    title: string;
    slug: string;
    uri: string;
    date: string;
    featuredImage: { node: MediaItem };
    readingTime: number;
};

const LatestNewsBlock = ({ data }: IFlexibleBlock<ILatestNewsBlock>) => {
    const [latestNews, setLatestNews] = useState<ILatestNewsBlock["insights"]["edges"]>([]);
    const [hoveredImageData, setHoveredImageData] = useState<InsightsItem["featuredImage"] | null>(null);
    const hoverImageRef = useRef<HTMLDivElement>(null);

    const handleMouseMove = (e: React.MouseEvent) => {
        if (hoverImageRef.current) {
            const { offsetWidth, offsetHeight } = hoverImageRef.current;
            gsap.to(hoverImageRef.current, {
                duration: 0.2,
                x: e.clientX - offsetWidth / 2,
                y: e.clientY - offsetHeight / 2,
                ease: "power3.out",
            });
        }
    };
    const { optionalTitle } = data.latestNewsFields || {};
    const titleToDisplay = optionalTitle && optionalTitle.trim() !== "" ? optionalTitle : "Other insight you might like";

    useEffect(() => {
        async function fetchData() {
            const newsData = await getLatestNews();
            setLatestNews(newsData);
        }
        fetchData();
    }, []);

    return (
        <div className="relative z-0 grid grid-cols-12 gap-x-20 px-30 lg:py-120 group">
            <div className="col-span-3 col-start-1 mb-80 mt-20">
                <p className="text-blue-dark dark:text-white text-24 font-medium leading-116 tracking-tight">{titleToDisplay}</p>
            </div>
            <div className="col-span-3 col-end-13 flex justify-end mb-80 mt-20">
                <AnimatedLink
                    href="/insights"
                    className="group flex items-center text-blue-dark dark:text-white text-21 leading-95 font-black tracking-tight uppercase"
                >
                    view all
                    <span className="ml-4 group-hover:rotate-45 group-hover:ml-6 duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none">
                            <path d="M1.07692 10L0 8.92308L7.38462 1.53846H0.769231V0H10V9.23077H8.46154V2.61538L1.07692 10Z" fill="#009BDF" />
                        </svg>
                    </span>
                </AnimatedLink>
            </div>
            {latestNews.map((insight) => (
                <AnimatedLink
                    key={insight.node.slug}
                    href={`/insights/${insight.node.slug}`}
                    // The border container is at z-0, so the image (z-10) can cover it.
                    className="relative z-0 col-start-1 col-span-12 lg:col-span-10 lg:col-start-2 py-20 border-t border-gray-40"
                    onMouseEnter={() => setHoveredImageData(insight.node.featuredImage)}
                    onMouseLeave={() => setHoveredImageData(null)}
                    onMouseMove={handleMouseMove}
                >
                    <div className="grid grid-cols-10 relative z-20">
                        <div className="col-start-1 col-span-12 lg:col-span-7 lg:col-start-1 mb-20 lg:mb-0">
                            <h3 className="text-blue-dark dark:text-white text-20 font-black leading-85 uppercase">{insight.node.title}</h3>
                        </div>
                        <div className="col-start-1 col-span-2 lg:col-span-1 lg:col-start-9">
                            <p className="text-16 font-medium leading-137 text-blue-dark dark:text-white">
                                <FormatDate dateString={insight.node.date || ""} />
                            </p>
                        </div>
                        <div className="col-start-4 col-span-3 lg:col-span-1 lg:col-start-10">
                            <p className="flex items-center text-16 font-medium leading-137 text-blue-dark dark:text-white">
                                <span className="mr-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                        <mask id="mask0_233_2037" maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
                                            <rect width="24" height="24" fill="#D9D9D9" />
                                        </mask>
                                        <g mask="url(#mask0_233_2037)">
                                            <path
                                                d="M15.25 5C14.9 5 14.6042 4.87917 14.3625 4.6375C14.1208 4.39583 14 4.1 14 3.75C14 3.4 14.1208 3.10417 14.3625 2.8625C14.6042 2.62083 14.9 2.5 15.25 2.5C15.6 2.5 15.8958 2.62083 16.1375 2.8625C16.3792 3.10417 16.5 3.4 16.5 3.75C16.5 4.1 16.3792 4.39583 16.1375 4.6375C15.8958 4.87917 15.6 5 15.25 5ZM15.25 21.5C14.9 21.5 14.6042 21.3792 14.3625 21.1375C14.1208 20.8958 14 20.6 14 20.25C14 19.9 14.1208 19.6042 14.3625 19.3625C14.6042 19.1208 14.9 19 15.25 19C15.6 19 15.8958 19.1208 16.1375 19.3625C16.3792 19.6042 16.5 19.9 16.5 20.25C16.5 20.6 16.3792 20.8958 16.1375 21.1375C15.8958 21.3792 15.6 21.5 15.25 21.5ZM19.25 8.5C18.9 8.5 18.6042 8.37917 18.3625 8.1375C18.1208 7.89583 18 7.6 18 7.25C18 6.9 18.1208 6.60417 18.3625 6.3625C18.6042 6.12083 18.9 6 19.25 6C19.6 6 19.8958 6.12083 20.1375 6.3625C20.3792 6.60417 20.5 6.9 20.5 7.25C20.5 7.6 20.3792 7.89583 20.1375 8.1375C19.8958 8.37917 19.6 8.5 19.25 8.5ZM19.25 18C18.9 18 18.6042 17.8792 18.3625 17.6375C18.1208 17.3958 18 17.1 18 16.75C18 16.4 18.1208 16.1042 18.3625 15.8625C18.6042 15.6208 18.9 15.5 19.25 15.5C19.6 15.5 19.8958 15.6208 20.1375 15.8625C20.3792 16.1042 20.5 16.4 20.5 16.75C20.5 17.1 20.3792 17.3958 20.1375 17.6375C19.8958 17.8792 19.6 18 19.25 18ZM20.75 13.25C20.4 13.25 20.1042 13.1292 19.8625 12.8875C19.6208 12.6458 19.5 12.35 19.5 12C19.5 11.65 19.6208 11.3542 19.8625 11.1125C20.1042 10.8708 20.4 10.75 20.75 10.75C21.1 10.75 21.3958 10.8708 21.6375 11.1125C21.8792 11.3542 22 11.65 22 12C22 12.35 21.8792 12.6458 21.6375 12.8875C21.3958 13.1292 21.1 13.25 20.75 13.25ZM12 22C10.6167 22 9.31667 21.7375 8.1 21.2125C6.88333 20.6875 5.825 19.975 4.925 19.075C4.025 18.175 3.3125 17.1167 2.7875 15.9C2.2625 14.6833 2 13.3833 2 12C2 10.6167 2.2625 9.31667 2.7875 8.1C3.3125 6.88333 4.025 5.825 4.925 4.925C5.825 4.025 6.88333 3.3125 8.1 2.7875C9.31667 2.2625 10.6167 2 12 2V4C9.76667 4 7.875 4.775 6.325 6.325C4.775 7.875 4 9.76667 4 12C4 14.2333 4.775 16.125 6.325 17.675C7.875 19.225 9.76667 20 12 20V22ZM12 14C11.45 14 10.9792 13.8042 10.5875 13.4125C10.1958 13.0208 10 12.55 10 12C10 11.9167 10.0042 11.8292 10.0125 11.7375C10.0208 11.6458 10.0417 11.5583 10.075 11.475L8 9.4L9.4 8L11.475 10.075C11.5417 10.0583 11.7167 10.0333 12 10C12.55 10 13.0208 10.1958 13.4125 10.5875C13.8042 10.9792 14 11.45 14 12C14 12.55 13.8042 13.0208 13.4125 13.4125C13.0208 13.8042 12.55 14 12 14Z"
                                                fill="#13284C"
                                            />
                                        </g>
                                    </svg>
                                </span>
                                <span>{insight.node.readingTime} mins</span>
                            </p>
                        </div>
                    </div>
                </AnimatedLink>
            ))}

            {hoveredImageData && (
                <HoverPortal>
                    <motion.div
                        ref={hoverImageRef}
                        className="fixed top-0 left-0 z-10 pointer-events-none mix-blend-difference"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                    >
                        <DynamicImage data={hoveredImageData} className="aspect-[480/300] max-w-[480px] mix-blend-saturation invert dark:invert-0" />
                    </motion.div>
                </HoverPortal>
            )}
        </div>
    );
};

export default LatestNewsBlock;
